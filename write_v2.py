...
def buidDeviceEntity():
	device_id = 'Sensor_0'
	entity_name = 'Milling Machine'
	entity_type = 'FirstMachineDevice'
	piece_name = 'Transfer_data_MPT_18_Impeller_0001_ref_APS_nok'
	...
	with open(my_file) as csvfile:
		...
		for row in reader:
			...
			for i in range(0,len(attributeName)):
				dicAttributes = { ty:"Integer", name:attributeName[i]}
				lista.append(dicAttributes)
...
def writeEntity():
	data = buidDeviceEntity()
	with open("provisionDevice1.json", "w") as f:
		f.write(data)

if __name__ == "__main__":
	writeMultipleMeasuresJsonFormat()
	writeEntity()
